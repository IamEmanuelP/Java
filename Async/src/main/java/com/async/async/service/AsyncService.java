...
import com.async.async.model.EmployeeAddresses;
import com.async.async.model.EmployeeNames;
import com.async.async.model.EmployeePhone;
import com.async.async.model.EmployeeType;
...
public class AsyncService {
	...
	@Async("asyncExecutor")
	public CompletableFuture<EmployeeType> getEmployeeType() throws InterruptedException
	{
		log.info("getEmployeeType Starts");
		EmployeeType employeeType = restTemplate.getForObject("http://localhost:8082/types", EmployeeType.class);

		log.info("employeeType, {}", employeeType);
		Thread.sleep(1000L);	//Intentional delay
		log.info("employeeType completed");
		return CompletableFuture.completedFuture(employeeType);
	}
